...
load ImageLocation.mat ImageLocation
load Array.mat StorageArray

StartTime = tic;

Target = 1;                                 %初始化第幾張圖片

pic = imread(ImageLocation{Target});
...
%輸出目前檔名
OutputImageNameText = ['現在：',ImageLocation{Target}];
...
%防止溢出
length(ImageLocation)
if Target < length(ImageLocation)
    Target = Target + 1;
end
